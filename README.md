...
<p align="center">
  <strong>A second brain for OpenCode.</strong><br />
  Project intelligence, token tracking, and invisible enforcement through a smart plugin. Zero workflow changes.
</p>
...
OpenWolf gives OpenCode a second brain: a file index so it knows what files contain before reading them, a learning memory that accumulates your preferences and past mistakes, and a token ledger that tracks everything. All through a smart plugin that fires on every action.
...
```
OpenClaw + AI             ██████████████████████████████████████  ~3.4M tokens
OpenCode CLI (no OpenWolf) ████████████████████████████████        ~2.5M tokens
OpenWolf + OpenCode CLI    ████████                                ~425K tokens
```

**OpenWolf saved ~80% of tokens** compared to bare OpenCode CLI on the same project.
...
详见 [LOCAL_INSTALL.md](./LOCAL_INSTALL.md)

That's it. Use `opencode` normally. OpenWolf is watching.
...
| `hooks/` | OpenCode plugin scripts (supports both OpenCode plugin and legacy hooks) |
| `config.json` | Configuration with sensible defaults |
| `identity.md` | Agent persona for this project |
| `OPENWOLF.md` | Instructions OpenCode follows every session |

## How It Works

Before OpenCode reads a file, OpenWolf tells it what the file contains and how large it is. If OpenCode already read that file this session, OpenWolf warns it. Before OpenCode writes code, OpenWolf checks your `cerebrum.md` for known mistakes. After every write, it auto-updates the project map and logs token usage. You see none of this. It just happens.
...
OpenCode updates this file when you correct it, express a preference, or make a decision. The Do-Not-Repeat list prevents the same mistake across sessions.
...
Every file gets a description and token estimate. OpenCode reads this instead of opening files when the summary is enough.
...
Before fixing anything, OpenCode checks if the fix is already known. After fixing, it logs the solution.
...
Capture full-page screenshots of your running app and let OpenCode evaluate the design.

```bash
openwolf designqc
```

Auto-detects your dev server, captures viewport-height JPEG sections of every route, and saves them to `.wolf/designqc-captures/`. Then tell OpenCode to read the screenshots and evaluate. Requires `puppeteer-core`.

## Reframe

Ask OpenCode to help you pick a UI framework. OpenWolf ships a curated knowledge base of 12 frameworks (shadcn/ui, Aceternity, Magic UI, DaisyUI, HeroUI, Chakra, Flowbite, Preline, Park UI, Origin UI, Headless UI, Cult UI) with battle-tested migration prompts. OpenCode reads `.wolf/reframe-frameworks.md`, asks you a few questions, and executes the migration with the right prompt for your project.

## How OpenWolf Compares

OpenWolf is not an AI wrapper. It is a smart plugin and a `.wolf/` directory. It doesn't run your AI for you or change your workflow. It gives OpenCode what it lacks: a project map so it reads less, a memory so it learns faster, and a ledger so you see where tokens go.
...
We were building products with OpenCode at Cytostack when we noticed something off. Sessions were eating through tokens faster than they should. When we dug in, we found OpenCode re-reading the same files multiple times, scanning entire directories to find one function, and having no way to know what a file contained without opening it. There was no project map, no read awareness, no token visibility. So we built the tooling we wished existed -- a file index so OpenCode reads less, a learning memory so it gets smarter, and a ledger that tracks every token. That became OpenWolf.
